<script>
  ...
  let commentTextareaRef = $state(null);
  ...
  // Initialize textarea height when the component mounts or comment changes
  $effect(() => {
    if (commentTextareaRef && comment) {
      tick().then(() => {
  ...
  // Handle comment input change and auto-resize textarea
